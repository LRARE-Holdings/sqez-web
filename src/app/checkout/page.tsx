  ...
  async function handleConfirm() {
    ...
    const returnUrl =
      typeof window !== "undefined"
        ? `${window.location.origin}/checkout?plan=${plan}&pending=1`
        : "/checkout";
...
export default function CheckoutPage() {
  ...
  const nextHref = useMemo(
    () => encodeURIComponent(`/checkout${queryString ? `?${queryString}` : ""}`),
    [queryString],
  );
      ...
      unsubDoc = onSnapshot(
        ...
        (snap) => {
          ...
          if (sessionId) {
            ...
            setDetail("Finalising your payment… this can take a few seconds.");
            return;
          }
        ...
        (err) => {
          console.error(err);
          setStatus("error");
          setDetail("We couldn’t confirm your payment yet. Please refresh.");
        },
      );
    });
        ...
        {status === "error" && (
          <>
            ...
            <div className="mt-6 flex flex-col gap-2 sm:flex-row">
              ...
              <Link href="/checkout" className="btn btn-outline w-full sm:w-auto">
                Back to checkout
              </Link>
            </div>
          </>
